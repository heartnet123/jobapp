...
import { isEmailAutomationDecision } from './email-automation.ts';
...
test('Contract Guard - EmailAutomationDecision', () => {
  const decision = {
    ...
    reason: 'The email confirms that an application was submitted.',
  };

  assert.strictEqual(isEmailAutomationDecision(decision), true, 'Valid email automation decision should pass');
  assert.strictEqual(isEmailAutomationDecision({ ...decision, category: 'newsletter' }), false, 'Invalid category should fail');
  assert.strictEqual(isEmailAutomationDecision({ ...decision, confidence: 1.2 }), false, 'Invalid confidence should fail');
  assert.strictEqual(isEmailAutomationDecision({ ...decision, recommendedAction: 'delete_email' }), false, 'Invalid action should fail');
});
