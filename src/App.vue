...
import type { JobApplication, Stage, Contact } from './types';
...
onMounted(async () => {
  try {
    ...
    if (res.ok) {
      ...
      throw new Error('Failed to fetch from SQLite');
    }
  } catch (err) {
    console.warn('SQLite daemon not connected. Falling back to localStorage...', err);
    const cached = localStorage.getItem('job_tracker_applications');
    if (cached) {
      try {
        const parsed = JSON.parse(cached);
        // Filter out preloaded mock applications so the user gets a clean slate immediately
        const mockIds = ['APP-042', 'APP-041', 'APP-040', 'APP-039', 'APP-038', 'APP-037', 'APP-036'];
        applications.value = parsed.filter((app: JobApplication) => !mockIds.includes(app.id));
      } catch {
        applications.value = [];
      }
    } else {
...
async function saveApplication(appData: JobApplication, isNew: boolean) {
  ...
  localStorage.setItem('job_tracker_applications', JSON.stringify(applications.value));
}
...
async function deleteApplication(id: string) {
  try {
    ...
    console.error('Failed to delete from SQLite database:', err);
  }
  // Local fallback cache
  localStorage.setItem('job_tracker_applications', JSON.stringify(applications.value));
}
    ...
    <main class="main-content">
      ...
      <header class="topbar">
        ...
        <div class="topbar-actions">
          ...
          <label class="search-field" v-if="currentView !== 'Analytics'">
